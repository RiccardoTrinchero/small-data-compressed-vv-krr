...
%
% This script reproduces the VV-KRR results for Example 3 S_cd, including:
% 1) the VV-KRR entry corresponding to model #2 in Table 6,
% 2) the scatter plot in Fig. 8 (right panel),
% 3) the parametric plot in Fig. 9 (bottom panel).
%
...
%   X_MC        : normalized training input samples for the full dataset;;
%   Y_MC        : training output responses for Scd for the full dataset;;
...
legend('Model #2', 'FontSize', FS, 'Location', 'southeast', 'FontName', 'Arial')
...
legend({'Reference', 'Model #2'}, 'FontSize', FS, 'Location', 'northwest', 'FontName', 'Arial')
...
% Inset plot (B Matrix)
% --------------------------
inset_pos = [0.63, 0.25, 0.30, 0.30];  % [x, y, width, height] in normalized units
